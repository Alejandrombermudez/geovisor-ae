...
export default function LeftSidebar({ activeCategory, onSelectCategory, onWidthChange, isMobile, proyecciones = [], activeProyeccionId = null, onSelectProyeccion, authUser, onLogout, onRequestLogin, onOpenIntro, metasYear: metasYearProp = 2026, onMetasYearChange, onOpenMetasMetrics, openMetasView = false, onMetasViewToggle, aliado = null, onOpenAliadoMetrics, onOpenAliadoDemo, onAliadoViewToggle, openAliadoView = false, onOpenPresentacion }: Props) {
  ...
  if (view === 'metas') {
    ...
    const VEREDAS_POR_AÑO: Record<number, string[]> = {
      ...
      2030: ['Bocagrande', 'San Gil', 'Vuelta del Gallo', 'Costa Rica'],
      2031: ['La Iberia', 'La Turbia Arriba', 'San Pablo', 'Santo Domingo'],
      2032: [],
    }
      ...
      <div style={sidebarBase}>
        ...
        <div className="geo-about-scroll" style={{ flex: 1, overflowY: 'auto', width: '100%', minHeight: 0 }}>
          <div style={{ padding: compact ? '0 4px 16px' : '0 12px 16px' }}>
            ...
            {!compact && (
              <div style={{ marginBottom: 12 }}>
                <div style={{ color: 'rgba(255,255,255,0.38)', fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 6 }}>
                  Año de intervención
                </div>
                <select
                  value={metasYear}
                  ...
                  style={{
                    ...
                    borderRadius: 9, color: '#fff', fontSize: 17, fontWeight: 700,
            ...
            <div style={{ marginBottom: 14 }}>
              {!compact && (
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 7 }}>
                  <span style={{ color: 'rgba(255,255,255,0.4)', fontSize: 16, fontWeight: 600 }}>{YEAR_MIN}</span>
                  <span style={{ color: METAS_COLOR, fontSize: 26, fontWeight: 800, letterSpacing: '-0.02em' }}>{metasYear}</span>
                  <span style={{ color: 'rgba(255,255,255,0.4)', fontSize: 16, fontWeight: 600 }}>{YEAR_MAX}</span>
                </div>
              )}
              ...
              {!compact && (
                <div style={{ display: 'flex', width: '100%', marginTop: 9, gap: '1%' }}>
                  {METAS_YEARS.map(y => (
                    <button
                      key={y}
  ...
  return (
    <div style={sidebarBase}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, minHeight: 0 }}>
        ...
        {/* Botón Metas (debajo de Conectividad) — sin ícono */}
        <>
          ...
          <button
            ...
            style={{
              background: hovered === 'metas' ? 'rgba(196,154,64,0.12)' : 'transparent',
              border: 'none', borderLeft: '4px solid transparent',
              color: hovered === 'metas' ? '#C49A40' : 'rgba(255,255,255,0.45)',
              cursor: 'pointer', display: 'flex', flexDirection: 'column',
              alignItems: 'center', justifyContent: 'center',
              gap: 5, width: '100%', padding: '16px 4px', transition: 'all 0.2s ease',
            }}
          >
            {showLabels && (
              <span style={{ fontSize: sidebarW > 160 ? 12 : 11, fontWeight: 700, lineHeight: 1.2, letterSpacing: '0.04em', textTransform: 'uppercase', textAlign: 'center', whiteSpace: 'nowrap' }}>
            ...
            {!showLabels && (
              <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.04em', textTransform: 'uppercase' }}>M</span>
            )}
          </button>
        </>
